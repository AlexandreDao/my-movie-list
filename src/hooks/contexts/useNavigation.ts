...
export const useBottomTabBarTotalHeight = () => {
  const { navigationState } = useContext(NavigationContext);

  if (navigationState.bottomTabBarTotalHeight === undefined) {
    throw new Error(
      "Couldn't find the bottom tab bar total height. Are you inside a screen in Bottom Tab Navigator?",
    );
  }

  return navigationState.bottomTabBarTotalHeight;
};
